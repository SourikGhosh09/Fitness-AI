...
from .sequencing import order_selected
VERSION='rules-0.1.1'
...
def adaptation(history):
    ...
    if all(h.get('missed',False) or (h['rpe'] is not None and h['rpe']>=9) for h in recent):
        return {'factor':0.9,'reason':'Three difficult sessions: reduce load and review recovery.'}
    if all(h['rpe'] is not None and h['rpe']<=6 and not h.get('missed',False) for h in recent):
...
def plan(profile,ready,candidates,history=None):
    ...
    items=order_selected(items,candidates,profile.goals)

...
export async function cache(key:string,value:unknown){const d=await database;await d.runAsync('INSERT OR REPLACE INTO cache(key,data) VALUES (?,?)',key,JSON.stringify(value));}
...
export async function clear(){const d=await database;await d.execAsync('DELETE FROM cache; DELETE FROM outbox;');await SecureStore.deleteItemAsync('session');}
